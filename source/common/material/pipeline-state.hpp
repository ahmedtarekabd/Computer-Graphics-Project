...
namespace our
{
    ...
    struct PipelineState
    {
        ...
        void setup() const
        {
            ...
            if (PipelineState::faceCulling.enabled)
            {
                glEnable(GL_CULL_FACE);
                glCullFace(PipelineState::faceCulling.culledFace);
                glFrontFace(PipelineState::faceCulling.frontFace);
            }
            else
            {
            ...
            if (PipelineState::depthTesting.enabled)
            {
                glEnable(GL_DEPTH_TEST);
                glDepthFunc(PipelineState::depthTesting.function);
            }
            else
            {
            ...
            if (PipelineState::blending.enabled)
            {
                glEnable(GL_BLEND);
                glBlendEquation(PipelineState::blending.equation);
                glBlendFunc(PipelineState::blending.sourceFactor, PipelineState::blending.destinationFactor);
                glBlendColor(PipelineState::blending.constantColor.x, PipelineState::blending.constantColor.y, PipelineState::blending.constantColor.z, PipelineState::blending.constantColor.w);
            }
            else
            {
                glDisable(GL_BLEND);
            }
            glColorMask(PipelineState::colorMask.x, PipelineState::colorMask.y, PipelineState::colorMask.z, PipelineState::colorMask.w);
            glDepthMask(PipelineState::depthMask);
        }
