...
namespace our
{
    ...
    glm::mat4 Entity::getLocalToWorldMatrix() const
    {
        ...
        glm::mat4 model_matrix = this->localTransform.toMat4(); // get this entity local transform matrix that is the transformation matrix from the entity's local space to the parent's
        Entity *p = this->parent;                               // get the parent
        while (p != nullptr)                                    // loop through each parentb untill reaching the root
        {
            model_matrix = p->localTransform.toMat4() * model_matrix; // muliply the matrices together to get modelmatrix
            p = p->parent;
        }
        return model_matrix; // return model matrix
    }
